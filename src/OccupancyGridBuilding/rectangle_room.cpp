...
RectangleRoom::RectangleRoom(int xFreeSpace, int yFreeSpace, const mapTools::Point &origin, const Map &parentMap) :
  ...
  upperLeft.x = (origin.x - xFreeSpace / 2) - 1;
  upperLeft.y = (origin.y - yFreeSpace / 2) - 1;

  lowerRight.x = (origin.x + xFreeSpace / 2) - 1;
  lowerRight.y = (origin.y + yFreeSpace / 2) - 1;
...
void RectangleRoom::populateMap() {
  ...
  upperRight.x = (origin.x + xFreeSpace / 2) - 1;
  upperRight.y = (origin.y - yFreeSpace / 2) - 1;

  mapTools::Point lowerLeft;
  lowerLeft.x = (origin.x - xFreeSpace / 2) - 1;
  lowerLeft.y = (origin.y + yFreeSpace / 2) - 1;
